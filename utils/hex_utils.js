...
const WORD_CEIL = BigNumber.from(2).pow(WORD_LENGTH * 8)
...
function concat(...args) {
  return ethUtil.bufferToHex(
    Buffer.concat(args.map((h) => ethUtil.toBuffer(h)))
  )
}
...
function leftPad(n, _size = WORD_LENGTH) {
  return ethUtil.bufferToHex(
    ethUtil.setLengthLeft(ethUtil.toBuffer(hexUtils.toHex(n)), _size)
  )
}
...
function rightPad(n, _size = WORD_LENGTH) {
  return ethUtil.bufferToHex(
    ethUtil.setLengthRight(ethUtil.toBuffer(hexUtils.toHex(n)), _size)
  )
}
...
function slice(n, start, end) {
  ...
  const sliceStart =
    start >= 0 ? start * 2 : Math.max(0, hex.length + start * 2)
